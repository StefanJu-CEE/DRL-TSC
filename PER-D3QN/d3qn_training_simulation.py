...
class D3QNSimulation:
    def __init__(
        ...
        waiting_time_penalty_scale: float = 0,  # 等待时间惩罚缩放系数
        ...
        # ===== 放行量正奖励（新增） =====
        flow_reward_weight: float = 0.15,         # 放行量奖励权重（建议 0.02~0.08 微调）
        # ===== 全局效率参数（新增） =====
        global_efficiency_weight: float = 0.8,      # 全局效率奖励权重
        efficiency_threshold: float = 10.0,         # 效率改善阈值
        efficiency_bonus: float = 2.0,              # 效率改善额外奖励
        efficiency_penalty_multiplier: float = 1.5, # 效率下降惩罚倍数
    ):
        self._Model = Model
        self._Memory = Memory
        ...
        # 全局效率相关参数（新增）
        self._global_efficiency_weight = float(global_efficiency_weight)
        self._efficiency_threshold = float(efficiency_threshold)
        self._efficiency_bonus = float(efficiency_bonus)
        self._efficiency_penalty_multiplier = float(efficiency_penalty_multiplier)
    ...
    def run(self, episode: int, epsilon: float):
        ...
        while self._step < self._max_steps:
            ...
            # ★ 全局效率奖励：基于等待时间的全局优化
            global_efficiency_bonus = self._compute_global_efficiency_reward(total_wait, next_total_wait)
            ...
            # 总奖励
            reward = base_reward + congestion_penalty + global_efficiency_bonus + flow_bonus
            ...
            if self._step < 100 and self._step % 20 == 0:
                print(f"[Debug] step={self._step} Δwait={waiting_time_change:.2f} "
                      ...
                      f"global_eff={global_efficiency_bonus:.2f} flow={flow_bonus:.2f} total={reward:.2f}")
    ...
    def _compute_global_efficiency_reward(self, current_wait: float, next_wait: float) -> float:
        """
        计算基于全局等待时间的效率奖励
        关注整体交通效率，而不仅仅是局部放行量
        """
        # 等待时间改善程度
        wait_improvement = current_wait - next_wait
        
        # 基础效率奖励：等待时间减少越多，奖励越高
        efficiency_reward = wait_improvement * self._global_efficiency_weight
        
        # 额外奖励：当等待时间显著减少时给予额外激励
        if wait_improvement > self._efficiency_threshold:
            efficiency_reward += self._efficiency_bonus
        
        # 惩罚：当等待时间增加时给予额外惩罚
        if wait_improvement < 0:
            efficiency_reward *= self._efficiency_penalty_multiplier
