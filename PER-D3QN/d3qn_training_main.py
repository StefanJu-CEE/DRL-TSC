...
def read_extra_from_ini(path="training_settings.ini"):
    ...
    if cp.has_section("reward"):
        extra["flow_reward_weight"] = cp["reward"].getfloat("flow_reward_weight", fallback=0.15)
        # 新增全局效率参数
        extra["global_efficiency_weight"] = cp["reward"].getfloat("global_efficiency_weight", fallback=0.8)
...
if __name__ == "__main__":
    ...
    sumo_cmd = set_sumo(cfg["gui"], cfg["sumocfg_file_name"], cfg["max_steps"])
    ...
    Simulation = D3QNSimulation(
        ...
        # 新增全局效率参数 ↓↓↓
        global_efficiency_weight=cfg.get("global_efficiency_weight", extra.get("global_efficiency_weight", 0.1)),
        efficiency_threshold=cfg.get("efficiency_threshold", extra.get("efficiency_threshold", 10.0)),
        efficiency_bonus=cfg.get("efficiency_bonus", extra.get("efficiency_bonus", 2.0)),
        efficiency_penalty_multiplier=cfg.get("efficiency_penalty_multiplier", extra.get("efficiency_penalty_multiplier", 1.5)),
    )
